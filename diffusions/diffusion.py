...
class DiffusionRunner:
    ...
    def restore_parameters(self, device: Optional[torch.device] = None) -> None:
        ...
        model_ckpt = torch.load(os.path.join(checkpoints_folder, 'model.pth'), map_location=device)
        self.model.load_state_dict(model_ckpt)

        ema_ckpt = torch.load(os.path.join(checkpoints_folder, 'ema.pth'), map_location=device)
    ...
    def calc_loss(self, clean_x: torch.Tensor, eps: float = 1e-5) -> Union[float, torch.Tensor]:
        ...
        """
        dev = clean_x.device
        t = self.sample_time(clean_x.shape[0], eps=eps).to(dev)

        mean, std = self.sde.marginal_prob(clean_x, t)

        z = torch.randn(clean_x.shape).to(dev)
        noise = z * std + mean

        score = self.calc_score(noise, t)
        loss = (z + score * std) ** 2
        loss = loss.mean()
    ...
    def set_conditional_sampling(
        ...
        def new_score_fn(x, t, y):
            ...
            """
            posterior_score_T = self.calc_score(x, t) + classifier_grad_fn(x, t, y) / T
    ...
    def set_classifier(self, classifier: torch.nn.Module, T: float = 1.0) -> None:
        ...
        def classifier_grad_fn(x, t, y):
            ...
            with torch.enable_grad():
                p = classifier(x, t)
                log_sm = F.log_softmax(p.view(x.shape[0], -1), dim=1)
                right_weights = log_sm[np.arange(y.shape[0]), y]

                likelihood_score = torch.autograd.grad(torch.sum(right_weights), x)[0]
    ...
    def sample_images(
        ...
        with torch.no_grad():
            ...
            ts = torch.linspace(self.sde.T, eps, self.sde.N, device=device)

            pred_images = None
            for i in range(self.sde.N):
                input_t = torch.ones(shape[0], device=device) * ts[i]
                x, pred_images = self.diff_eq_solver.step(x, input_t, labels)
    ...
    def train_classifier(
        ...
        def get_logits(X, y):
            ...
            noise = torch.randn(mean.shape, device=device) * std + mean
            logits = classifier(noise, t)
        ...
        for iter_idx in trange(1, 1 + self.config.classifier.training_iters):
            ...
            self.log_metric('CrossEntropy', 'train', loss.item())
            self.log_metric('Acc', 'train', (labels == y).float().mean().item())
            ...
            if iter_idx % self.config.classifier.eval_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    for x, y in self.datagen.valid_loader:
                        ...
                        valid_accuracy += torch.sum(labels == y).item()

                valid_loss = valid_loss / valid_count
                valid_accuracy = valid_accuracy / valid_count
                self.log_metric('CrossEntropy', 'valid', valid_loss)
                self.log_metric('Acc', 'valid', valid_accuracy)
                classifier.train()
